...
function managedFile(): string {
	return sanitizeSidecarFile(prefs()["managed-file"] || SIDECAR_DEFAULT);
}
...
function tileIcon(bg: string, badge: "none" | "star" | "slash"): string {
	...
	const slash =
		badge === "slash"
			? `<line x1="15" y1="49" x2="49" y2="15" stroke="#fff" stroke-width="5" stroke-linecap="round"/>`
			: "";
...
function mergeOrder(prev: string[], monitors: HyprMonitor[]): string[] {
	...
	if (kept.length > 0 || fresh.length !== enabled.length)
		return [...kept, ...fresh];
	return orderLeftToRight(monitors.filter((m) => !m.disabled)).map(
		(m) => m.name,
	);
}
...
function EditPosition({
	...
	return (
		...
		>
			<Form.Description
				title="Display"
				text={`${monitor.name} — ${modeFor(monitor)}`}
			/>
			<Form.TextField
				id="x"
				title="X position"
				...
				onChange={(v) =>
					setXError(
						parseCoordinate(v) === null
							? "Integer pixels, e.g. 2560"
							: undefined,
					)
				}
			/>
			<Form.TextField
				...
				onChange={(v) =>
					setYError(
						parseCoordinate(v) === null ? "Integer pixels, e.g. 0" : undefined,
					)
				}
			/>
			<Form.Description
				title="Note"
				text="Applies instantly via hyprctl. Negative Y places above, like your vertical offsets."
			/>
		</Form>
	);
}
...
export default function Command() {
	...
	const load = useCallback(async (announceLegacy: boolean) => {
		try {
			const [monitors, cfg] = await Promise.all([
				getAllMonitors(),
				getConfigState(),
			]);
			setError(null);
	...
	const refreshMonitors = useCallback(async () => {
		...
		try {
			...
			if (hasLiveData.current && !pollErrorShown.current) {
				pollErrorShown.current = true;
				await showToast({
					style: Toast.Style.Failure,
					title: "Display refresh failed",
					message: `Showing last known layout. ${e instanceof Error ? e.message : String(e)}`,
				});
			}
		}
	}, []);
	...
	const enabled = useMemo(
		() =>
			order
				.map((n) => byName.get(n))
				.filter((m): m is HyprMonitor => Boolean(m)),
		[order, byName],
	);
	const disabled = useMemo(() => (all ?? []).filter((m) => m.disabled), [all]);
	const placements = useMemo(() => tileHorizontallyKeepY(enabled), [enabled]);
	...
	const guarded = useCallback(
		async (fn: () => Promise<void>, ok?: string, message?: string) => {
			...
			try {
				...
				await load(false);
				if (ok)
					await showToast({ style: Toast.Style.Success, title: ok, message });
			} catch (e) {
				await showToast({
					style: Toast.Style.Failure,
					title: "Hyprland call failed",
	...
	const applyAndPersist = useCallback(
		async (placements: PlacedMonitor[], ok: string) => {
			...
			await showToast({
				...
				title: ok,
				message: r.changedConfig
					? "Config updated — layout persists across reboots"
					: !r.persisted
						? "Session only — switch to hyprland.lua for persistence"
						: undefined,
			});
		},
		[config],
	...
	const move = useCallback(
		(name: string, dir: -1 | 1) => {
			...
			const ordered = next
				.map((n) => byName.get(n))
				.filter((m): m is HyprMonitor => Boolean(m));
			const nextPlacements = tileHorizontallyKeepY(ordered);
			void guarded(async () => {
				await applyAndPersist(
					nextPlacements,
					`Moved ${name} ${dir === -1 ? "left" : "right"}`,
				);
				setOrder((prev) => {
	...
	if (placements.length === 0 && disabled.length === 0) {
		return (
			<List searchBarPlaceholder="Displays — Ctrl+←/→ to reorder">
				<List.EmptyView
					icon={{ source: Icon.Monitor, tintColor: Color.SecondaryText }}
					title="No displays found"
					description="hyprctl reported zero monitors. Reconnect a display, then reload."
					actions={
						<ActionPanel>
							<Action
								title="Reload"
	...
	return (
		<List
			isLoading={busy}
			isShowingDetail
			searchBarPlaceholder="Displays — Ctrl+←/→ to reorder"
		>
			{config.isLegacy ? (
						...
						actions={
							<ActionPanel>
								<Action.OpenInBrowser
									title="Open Lua Migration News"
									url="https://hypr.land/news/26_lua"
								/>
								<Action
									title="Reload"
									icon={Icon.ArrowClockwise}
							...
							actions={
								<ActionPanel>
									...
									<Action
										...
										onAction={() => {
											if (enabled.length <= 1) {
												void showToast({
													style: Toast.Style.Failure,
													title: "Cannot disable the last active display",
												});
												return;
											}
											void guarded(async () => {
									...
									<ActionPanel.Submenu title="Actions" icon={Icon.Ellipsis}>
											...
											target={
												<EditPosition
													...
													onApply={(nx, ny) =>
														guarded(() =>
															applyAndPersist(
																withRepositioned(placements, m.name, nx, ny),
																`Moved ${m.name} to ${nx}x${ny}`,
															),
														)
													}
												/>
										...
										/>
										<Action.ShowInFinder
											title="Reveal Hypr Config Folder"
											path={config.hyprDir}
										/>
										<Action
											...
											onAction={() => {
												void (async () => {
													...
													await guarded(async () => {
														...
														await showToast({
															style: Toast.Style.Success,
															title: "Extension config removed",
														});
													});
												})();
											}}
				...
				<List.Section title={`Disabled — ${disabled.length}`}>
					{disabled.map((m) => (
						<List.Item
							...
							detail={
								<List.Item.Detail
									markdown={`# ${m.name}\n\nDisabled. Enable it, then reorder with Ctrl+Left / Ctrl+Right.`}
								/>
							}
							actions={
								<ActionPanel>
