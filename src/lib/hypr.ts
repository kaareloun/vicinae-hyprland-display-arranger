...
export function stableOutput(m: HyprMonitor): string {
	if (m.serial && m.description && m.description.includes(m.serial))
...
export function evalForPlacement(m: HyprMonitor, x: number, y: number): string {
	return `hl.monitor({ output = ${luaStr(m.name)}, mode = ${luaStr(modeFor(m))}, position = ${luaStr(`${x}x${y}`)}, scale = ${scaleFor(m)}, disabled = false })`;
}
...
export function legacyKeywordFor(m: HyprMonitor, x: number, y: number): string {
	return `monitor=${m.name},${modeFor(m)},${x}x${y},${scaleFor(m)}`;
}
...
export async function setEnabled(
	...
	if (provider === "lua") {
		...
		await run("hyprctl", [
			"eval",
			`hl.monitor({ output = ${luaStr(m.name)}, mode = ${luaStr(modeFor(m))}, position = "auto", scale = ${scaleFor(m)}, disabled = false })`,
		]);
		return;
	}
	if (!enabled) {
	...
	await run("hyprctl", [
		"keyword",
		`monitor=${m.name},${modeFor(m)},auto,${scaleFor(m)}`,
	]);
}
...
export function liveEntry(p: PlacedMonitor): SidecarEntry {
	return {
		...
		text: `hl.monitor({ output = ${luaStr(stableOutput(p.monitor))}, mode = ${luaStr(modeFor(p.monitor))}, position = ${luaStr(`${p.x}x${p.y}`)}, scale = ${scaleFor(p.monitor)} })`,
	};
}
...
export async function persistLiveLayout(
	...
	if (!entries.some((e) => e.output === "")) {
		entries.push({
			output: "",
			text: `hl.monitor({ output = "", mode = "preferred", position = "auto", scale = 1 })`,
		});
	}
